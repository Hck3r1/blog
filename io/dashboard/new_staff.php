...
<body>
    ...
    if(isset($_REQUEST['submit'])){
        ...
        $gender=trim(addslashes($_REQUEST['gender']));
			...
			$check=mysqli_query($conn, "SELECT * FROM admin WHERE phone='$phone' AND email='$emailreg' AND designation='Staff'");
    $checkrows=mysqli_num_rows($check);
    if($checkrows>0) {
        echo "<script>alert('User already exist')
        location.href='new_staff.php'</script>";
        
    } else
    
    {
        $sql="INSERT INTO admin (uin, name, email, phone, password, passport, designation, gender) VALUES ('$uin','$name','$emailreg','$phone',PASSWORD('$passwordreg'),'$passport','Staff','$gender')";
		...
		echo "<script>alert('Staff has been registered successfully.')</script>";
						...
						?>
                                        <div>
                                            ...
                                            <div class="form-group"><label>GENDER</label> <input type="text" placeholder="Enter your gender" class="form-control" name="gender"></div>
